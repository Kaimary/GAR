...
from ..sqlgen.sqlgen import Generator
from ..sqlgen.utils.sql_tmp_update import sql_nested_query_tmp_name_convert
...
def generate_rerank_data_from_spider_dataset(
        ...
        candidate_num=80, overwrite=0
):
    # for GEO dataset, we augment the training data 2 times
    if dataset_name == 'geo' and not dev:
        candidate_num = 130
    output = []
    ...
    with open(dataset_file, "r") as data_file:
        ...
        for ex in data:
            ...
            if dataset_name == "geo" and not dev:
                for j in range(0, 2):
                    start = j * candidate_num
                    end = (j + 1) * candidate_num
                    ...
                    output.append(ins)
            elif not dev or (dev and max_instances and dev_count < max_instances):
