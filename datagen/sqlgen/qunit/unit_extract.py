...
from config import DIR_PATH, QUNITS_FILE, QUNITS_SET_COVER_FILE
from spider_utils.utils import DBSchema, get_all_schema
...
def extract_spider_unit(dataset='spider', schema_file='sqlgenv2/datasets/spider/tables.json',
    ...
    # check dir exists
    dir_path = (DIR_PATH + QUNITS_FILE.format(dataset, stage)).rsplit('/', 1)[0]
    if not os.path.exists(dir_path):
        os.mkdir(dir_path)

    with open(DIR_PATH + QUNITS_FILE.format(dataset, stage), 'w') as f:
        json.dump(spider_patterns, f, indent=4)
    with open(DIR_PATH + QUNITS_SET_COVER_FILE.format(dataset, stage), 'w') as f:
