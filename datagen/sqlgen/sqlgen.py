...
from config import DIR_PATH, QUNITS_FILE, TIME_LIMIT_PRE_SQL
from datagen.sqlgen.qunit.unit_extract import extract_spider_unit
from spider_utils.evaluation.evaluate import rebuild_sql
from spider_utils.utils import disambiguate_items2, read_single_dataset_schema
...
from .classes.rule_set import RULE_SET
from .classes.combinatorial_rule import COMBINATORIAL_RULE_DICTIONARY


class Generator(object):
    def __init__(self, dataset_name, db_name, db_schema, data_file, tables_file, db_dir, kmaps, stage):
        """
        :param db_name: The name of database that needs to generate data.
        :param db_schema: The schema info(DBSchema) of current database.
        :param db_dir: The directory of databases for the dataset.
        :param kmaps: The built foreign key map from schema file.
        """
    ...
    def init_dataset(self):
        db_file = os.path.join(self.db_dir, self.db_name, self.db_name + ".sqlite")
        if not os.path.isfile(db_file):
            self.schema = get_schema_from_json(self.db_name, self.tables_file)
        else:
        ...
        if not os.path.exists(DIR_PATH + QUNITS_FILE.format(self.dataset_name, self.stage)):
            print(
                f"Not found query units file! Please check if the file path setting is correct: "
                f"{QUNITS_FILE.format(self.dataset_name, self.stage)}")
            print(f"Extract the query units...")
            extract_spider_unit(self.dataset_name, self.tables_file, self.data_file, db_path=self.db_dir, stage=self.stage)
            print(f"Extract the query units complete!")
        with open(DIR_PATH + QUNITS_FILE.format(self.dataset_name, self.stage), "r") as data_file:
    ...
    def generate_sql(self, sql_num):
        # Firstly to determine the SQL skeleton types.
                ...
                if ret:
                    ...
                    if is_valid:
                        ...
                        if sql_map in sql_map_set:
                            ...
                            _, sql_dict, schema_ = disambiguate_items2(tokenize(sql1), self.schema, self.table,
                                                                       allow_aliases=False)
                            dialect = convert_sql_to_dialect(sql_dict, self.table_dict, schema_)
