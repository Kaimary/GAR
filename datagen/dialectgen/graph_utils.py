...
from spider_utils.evaluation.process_sql import WHERE_OPS, AGG_OPS
from config import USE_ANNOTATION
"""
Generic Templates
"""
TEMPLATE = {
...
def build_graph_from_sql(G, sql_dict, table_dict, schema, parent_op=None, parent_node=None):
    # Read USE_ANNOTATION flag from config to determine if using annotations for dialect generation.
    ...
    # If join exists
        ...
        for i, (t1, t2, cond) in enumerate(
                zip(from_dict['table_units'][:-1], from_dict['table_units'][1:], from_dict['conds'][::2])):
            _, table_id1 = t1
