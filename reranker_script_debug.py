...
# @click.argument("log_file", type=click.Path(exists=False, dir_okay=False))
# @click.argument("output_file", type=click.Path(exists=False, dir_okay=False))
def main(dataset_name, dataset_file, tables_file, db_dir, mode):
    # Initialization
    output = []
    total = 0
    db_count = 0
    ...
    output_dir = f'{DIR_PATH}{OUTPUT_DIR.format(dataset_name, GENERATION_NUM, CANDIDATE_NUM, RETRIEVAL_MODEL_NAME, f"{RERANKER_MODEL_NAME}+{RERANKER_EMBEDDING_MODEL_NAME}")}'
    ...
    with open(dataset_file, "r") as data_file:
        ...
        for i, ex in enumerate(data):
            ...
            gold_dialect = synthesizer.synthesize(gold_sql) if not 'from (' in gold_sql else 'NA'
